...
import AddEditCompte from "./AddGouvernorat";
...
export default function RueList() {
  ...
  const fetchComptes = async () => {
    try {
      const response = await axios.get("http://localhost:5000/gouvernorats/get");
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/regions/${id}`);
      fetchComptes();
    } catch (error) {
